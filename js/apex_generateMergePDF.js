...
window.pdfFetchedBlobs = window.pdfFetchedBlobs || [];

class processPDF {
    constructor() {
        // Inicializa as variáveis como vazias
        window.pdfGeneratedBlob = null;
        window.pdfFetchedBlobs = [];
        this.atualizarConteudoInstance = new processPDF.AtualizarConteudo();
    }

    async runProcess(num_pag) {
        await this.atualizarConteudoInstance.atualizarConteudo(num_pag);
    ...
    async generatePDF() {
        ...
        // Chama a função para processar os anexos
        await this.processarAnexos();

        await this.mergerPDF();
    }

    // Função para buscar o PDF de acordo com o ANEXO_ID
    async fetchPdf(anexoId) {
        // Construir a URL dinâmica
        const fileUrl = $('#P34_URL_APEX').val(); // Verifique se o seletor está correto
        ...
        try {
            ...
            window.pdfFetchedBlobs.push(blob);
    ...
    async processarAnexos() {


        const v_input = document.getElementById('P34_AUX_ARRAY_ANEXO').value;
        if (v_input.trim() !== '') {
            ...
            console.log('Todos os blobs foram armazenados:', window.pdfFetchedBlobs);
        } else {
    ...
    async mergerPDF() {
        ...
        if (window.pdfGeneratedBlob) {
            // Adicionar o PDF gerado ao array
            pdfDocs.push(window.pdfGeneratedBlob);
        }
        
        if (window.pdfFetchedBlobs && window.pdfFetchedBlobs.length > 0) {
            // Adicionar os PDFs vindos do banco de dados ao array  
            window.pdfFetchedBlobs.forEach(blob => {  
                pdfDocs.push(blob);
            });
        }
...
processPDF.AtualizarConteudo = class {
    constructor() {
        this.contador = 1;
    }
